...
public class WidgetTabItemLister extends GuiAnimatedStat {
    ...
    public WidgetTabItemLister(GuiScreen gui, String title, ItemStack icon, int xPos, int yPos, int backGroundColor, IGuiAnimatedStat affectingStat,
            boolean leftSided) {
        super(gui, title, icon, xPos, yPos, backGroundColor, affectingStat, leftSided);
    }

    public WidgetTabItemLister(GuiScreen gui, String title, String texture, int xPos, int yPos, int backGroundColor, IGuiAnimatedStat affectingStat,
            boolean leftSided) {
    ...
    public void render(FontRenderer fontRenderer, float zLevel, float partialTicks) {
        ...
        if (isDoneExpanding()) {
            if (showingItems.size() > 0) {
                Gui.drawRect(getBaseX() + 7, getAffectedY() + 16, getBaseX() + Math.min(MAX_ITEMS_X, showingItems.size()) * 18 + 9, getAffectedY()
                        + 36 + (showingItems.size() - 1) / MAX_ITEMS_X * 18, 0xFFAAAAAA);
            }
