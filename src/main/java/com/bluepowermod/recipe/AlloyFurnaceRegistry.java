...
public class AlloyFurnaceRegistry implements IAlloyFurnaceRegistry {
    ...
    public void addRecipe(FluidStack result, Object... requiredItems) {
        ...
        addRecipe(new StandardAlloyFurnaceRecipe(result, requiredStacks));
    }

    @Override
    public void addRecyclingRecipe(FluidStack recycledFluid, String... blacklist) {

        bufferedRecyclingItems.add(recycledFluid);
    ...
    public IAlloyFurnaceRecipe getMatchingRecipe(ItemStack[] input, FluidStack output) {

        for (IAlloyFurnaceRecipe recipe : alloyFurnaceRecipes) {
            if (recipe.matches(input)) {
                if (output != null) {
                    if (output != null) {// check if we can add the crafting result to the output slot
                        FluidStack result = recipe.getResult(input);
                        if (!FluidStack.areFluidStackTagsEqual(output, result) || !output.isFluidEqual(result)) {
                            continue;
                        } else if (result.amount + output.amount > TANK_SIZE) {
                            continue;
                        }
                    }
                }
    ...
    public class StandardAlloyFurnaceRecipe implements IAlloyFurnaceRecipe {
        ...
        private StandardAlloyFurnaceRecipe(FluidStack result, ItemStack... requiredItems) {
        ...
        @Override
        public FluidStack getResult(ItemStack[] input) {
