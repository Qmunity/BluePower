...
public abstract class BPPart {
    ...
    public boolean canPlacePart(ItemStack is, EntityPlayer player, Vector3 block, MovingObjectPosition mop) {
        ...
        for (AxisAlignedBB occBox : getOcclusionBoxes()) {
            if (checkOcclusion(occBox)) return false;
        }
        return true;
    }
    ...
    public boolean checkOcclusion(AxisAlignedBB cube) {
        ...
        return BPApi.getInstance().getMultipartCompat().checkOcclusion(world.getTileEntity(x, y, z), cube);
    }
