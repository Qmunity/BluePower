...
public class ItemCropSeed extends BlockItem {
    ...
    public InteractionResult useOn(UseOnContext itemUseContext) {
        ...
        if (facing.ordinal() != 1) {
            ...
            if (world.getBlockState(pos).getBlock().canSustainPlant(world.getBlockState(pos),  world, pos, Direction.UP, block.get().defaultBlockState()).isDefault() && world.isEmptyBlock(pos.above()) && world.getBlockState(pos).getBlock() instanceof FarmBlock) {
