...
public class BlockSortron extends BlockContainerBase {
     ...
     * 
     * 
     * TileSortron tile = (TileSortron) world.getTileEntity(x, y, z); ForgeDirection dir = tile.getFacingDirection(); if (dir.ordinal() == side) {
     * return icons[0]; } else if (dir.getOpposite().ordinal() == side) { return icons[1]; } else if ((side == 1 && dir.ordinal() != 1 &&
     * dir.getOpposite().ordinal() != 1) || (side == 3 && (dir.ordinal() == 1 || dir.getOpposite().ordinal() == 1))) { // && isPowered return
     * icons[6]; } else if (tile.showOutPutAnimation()) { // && isPowered return icons[4]; } else { // && isPowered return icons[3]; } //TODO:
     * different icons when powered }
     */
