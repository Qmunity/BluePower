...
public class BPBlockMultipart extends BlockContainer {
    ...
    public void addCollisionBoxesToList(World w, int x, int y, int z, AxisAlignedBB unused, List l, Entity entity) {
        ...
        for (AxisAlignedBB aabb : t.getCollisionBoxes()) {
            ...
            aabb.maxY += y;
            aabb.maxZ += z;
            if (aabb.intersectsWith(unused)) l.add(aabb);
        }
    }
    ...
    public MovingObjectPosition collisionRayTrace(World w, int x, int y, int z, Vec3 start, Vec3 end) {
        ...
        if (t != null) {
            ...
            if (mop != null) setBlockBounds(mop.getCubeHit());
        }
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World w, int x, int y, int z) {
        ...
        BPTileMultipart t = getTile(w, x, y, z);
        if (t == null) return super.getSelectedBoundingBoxFromPool(w, x, y, z);
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (v.getBlock() != this) return;
        BPTileMultipart te = (BPTileMultipart) v.getTileEntity();
        if (te == null) return;
        BPMop mop = rayTrace(event.world, event.x, event.y, event.z, RayTracer.getStartVector(event.entityPlayer), RayTracer.getEndVector(event.entityPlayer), te.getParts());
    ...
    public static BPMop rayTrace(World w, int x, int y, int z, Vector3 start, Vector3 end, List<AxisAlignedBB> aabbs, boolean unused) {
        ...
        for (AxisAlignedBB aabb : aabbs) {
            ...
            Blocks.stone.setBlockBounds((float) aabb.minX, (float) aabb.minY, (float) aabb.minZ, (float) aabb.maxX, (float) aabb.maxY, (float) aabb.maxZ);
    ...
    public ItemStack getPickBlock(MovingObjectPosition target, World w, int x, int y, int z) {
        ...
        BPMop mop = rayTrace(w, x, y, z, RayTracer.getStartVector(Minecraft.getMinecraft().thePlayer), RayTracer.getEndVector(Minecraft.getMinecraft().thePlayer), te.getParts());
    ...
    public boolean removedByPlayer(World w, EntityPlayer player, int x, int y, int z, boolean willHarvest) {
        ...
        if (te.getParts().size() == 0) return true;
