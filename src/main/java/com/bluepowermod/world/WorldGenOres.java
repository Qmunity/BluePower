    ...
    private static void addOreToGenerate(int veinCount, int veinSize, int minY, int maxY, Block ore){
        for(Biome biome : ForgeRegistries.BIOMES) {
            if(!biome.getCategory().equals(Biome.Category.NETHER) && !biome.getCategory().equals(Biome.Category.THEEND)) {
                biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.withConfiguration(new OreFeatureConfig(NATURAL_STONE, ore.getDefaultState(), veinSize)).withPlacement(COUNT_RANGE.configure(new CountRangeConfig(veinCount, minY, minY, maxY))));
            }
        }
    }
