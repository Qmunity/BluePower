...
import com.bluepowermod.api.wire.redstone.IRedstoneConductor2;
...
public abstract class PartRedwireFace extends PartWireFace implements IRedwire, IIntegratedCircuitPart, IPartRedstone {
    ...
    protected boolean shouldRenderConnection(ForgeDirection side) {
        ...
        if (getParent() == null || getWorld() == null) return isConnected(side);
    ...
    public static class PartRedwireFaceUninsulated extends PartRedwireFace implements IRedstoneConductor2, IConnectionListener, IPartTicking {
        ...
        public boolean canConnect(ForgeDirection side, IRedstoneDevice device, ConnectionType type) {
            ...
            if ((type == ConnectionType.STRAIGHT && side == getFace().getOpposite() && device instanceof IFace) || side == ForgeDirection.UNKNOWN) return false;
            if (type == ConnectionType.CLOSED_CORNER) {
                if (side == getFace()) return false;
                if (side == getFace().getOpposite()) return false;
            ...
            if (!OcclusionHelper.microblockOcclusionTest(getParent(), MicroblockShape.EDGE, 2, getFace(), side)) return false;
        ...
        public byte getRedstonePower(ForgeDirection side) {
            ...
            if (!RedstoneApi.getInstance().shouldWiresOutputPower(hasLoss(side))) return 0;
            
            if (!isAnalogue(side)) return (byte) ((power & 0xFF) > 0 ? 255 : 0);
        ...
        public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
            ...
            GL11.glPushMatrix();
            {
                if (type == ItemRenderType.ENTITY) GL11.glTranslated(droppedTranslation, 0, droppedTranslation);
                    ...
                    if (dev instanceof IFace && ((IFace) dev).getFace() == ForgeDirection.getOrientation(i).getOpposite()) {
                        if (dev instanceof IRedwire) {
                            if (dev instanceof IInsulatedRedstoneDevice
                                    && ((IInsulatedRedstoneDevice) dev).getInsulationColor(c.getSideB()) != MinecraftColor.NONE) render = true;
                            if (dev instanceof IFace && getFace().ordinal() > ((IFace) dev).getFace().ordinal()) render = true;
                        } else {
                            connected = true;
                            render = true;
                        }
        ...
        public void readUpdateData(DataInput buffer) throws IOException {
            ...
            if (getParent() != null && getWorld() != null) getWorld().markBlockRangeForRenderUpdate(getX(), getY(), getZ(), getX(), getY(), getZ());
        }
        ...
        public int getWeakPower(ForgeDirection side) {
            ...
            // return 0;
            if (side == getFace().getOpposite()) return 0;
            if (!OcclusionHelper.microblockOcclusionTest(getParent(), MicroblockShape.EDGE, 1, getFace(), side)) return 0;
    ...
    public static class PartRedwireFaceInsulated extends PartRedwireFace implements IAdvancedRedstoneConductor, IInsulatedRedstoneDevice,
        ...
        protected boolean isConnected(ForgeDirection side) {
            ...
            if (getParent() == null || getWorld() == null) return true;
        ...
        public boolean canConnect(ForgeDirection side, IRedstoneDevice device, ConnectionType type) {
            ...
            if (device instanceof IInsulatedRedstoneDevice) {
                MinecraftColor c = ((IInsulatedRedstoneDevice) device).getInsulationColor(type == ConnectionType.STRAIGHT ? side.getOpposite()
                        : (type == ConnectionType.CLOSED_CORNER ? getFace() : getFace().getOpposite()));
                if (c != null && c != getInsulationColor(side)) return false;
            }
        ...
        public boolean canConnect(ForgeDirection side, IBundledDevice device, ConnectionType type) {
            ...
            if (device instanceof IInsulatedRedstoneDevice) return false;
        ...
        public byte[] getBundledOutput(ForgeDirection side) {
            ...
            if (!RedstoneApi.getInstance().shouldWiresOutputPower(hasLoss(side))) return new byte[16];
        ...
        public void onRedstoneUpdate() {
            ...
            if (getParent() == null || getWorld() == null) return;
            ...
            if (hasUpdated) {
                ...
                for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if (c == null) continue;
                    IRedstoneDevice dev = c.getB();
                    if (dir == getFace()) RedstoneHelper.notifyRedstoneUpdate(getWorld(), getX(), getY(), getZ(), dir, true);
                    else if ((dev == null || dev instanceof DummyRedstoneDevice) && dir != getFace().getOpposite()) RedstoneHelper
                            .notifyRedstoneUpdate(getWorld(), getX(), getY(), getZ(), dir, false);
                }
        ...
        public void propagate(ForgeDirection fromSide, Collection<IConnection<IRedstoneDevice>> propagation) {
            ...
            for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (cB != null) propagation.add(new RedstoneConnection(this, BundledDeviceWrapper.wrap(cB.getB(), color), cB.getSideA(), cB
                        .getSideB(), cB.getType()));
            }
        }
        ...
        public boolean renderStatic(Vec3i translation, RenderHelper renderer, RenderBlocks renderBlocks, int pass) {
            ...
            // Sides
            if (s4 || s3) {
                if (s3 || (!s1 && !s2)) renderer.renderBox(new Vec3dCube(s3 ? (cornerConnect[d3.ordinal()] ? -height - size : 0.001) : 5 / 16D, y,
                        8 / 16D - width, 8 / 16D - width, height + size, 8 / 16D + width), IconSupplier.wire);
                if (s4 || (!s1 && !s2)) renderer.renderBox(new Vec3dCube(8 / 16D + width, y, 8 / 16D - width, s4 ? (cornerConnect[d4.ordinal()] ? 1
                        + height + size : 0.999) : 11 / 16D, height + size, 8 / 16D + width), IconSupplier.wire);
                if (s1) renderer.renderBox(new Vec3dCube(8 / 16D - width, y, s1 ? (cornerConnect[d1.ordinal()] ? -height - size : 0.001) : 4 / 16D,
                        8 / 16D + width, height + size, 8 / 16D - width), IconSupplier.wire);
                if (s2) renderer.renderBox(
            ...
            if (s4 || s3) {
                if (s3 || (!s1 && !s2)) renderer.renderBox(new Vec3dCube(4 / 16D - len, 0, 8 / 16D - width, 4 / 16D, 2 / 16D, 8 / 16D + width),
                        IconSupplier.wire);
                ...
                if (!s1) renderer.renderBox(new Vec3dCube(8 / 16D - width, 0, 4 / 16D - len, 8 / 16D + width, 2 / 16D, 4 / 16D), IconSupplier.wire);
                if (!s2) renderer.renderBox(new Vec3dCube(8 / 16D - width, 0, 12 / 16D, 8 / 16D + width, 2 / 16D, 12 / 16D + len), IconSupplier.wire);
            }
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (c != null) {
                    ...
                    if (dev instanceof IFace && ((IFace) dev).getFace() == ForgeDirection.getOrientation(i).getOpposite()) {
                        if (dev instanceof IRedwire) {
                            if (dev instanceof IInsulatedRedstoneDevice
                                    && ((IInsulatedRedstoneDevice) dev).getInsulationColor(c.getSideB()) != MinecraftColor.NONE) connected = true;
                            if (dev instanceof IFace && getFace().ordinal() > ((IFace) dev).getFace().ordinal()) {
                                if (dev instanceof IInsulatedRedstoneDevice
                                        && ((IInsulatedRedstoneDevice) dev).getInsulationColor(c.getSideB()) == getInsulationColor(c.getSideA())) render = true;
                                if (getInsulationColor(c.getSideA()) == MinecraftColor.NONE) render = true;
                            }
                        } else {
                            connected = true;
                            render = true;
                ...
                if (bc != null) {
                    ...
                    if (dev instanceof IFace && ((IFace) dev).getFace() == ForgeDirection.getOrientation(i).getOpposite()) {
                        if (dev instanceof IRedwire) {
                            if (dev instanceof IInsulatedRedstoneDevice
                                    && ((IInsulatedRedstoneDevice) dev).getInsulationColor(bc.getSideB()) != MinecraftColor.NONE) connected = true;
                            if (dev instanceof IFace && getFace().ordinal() > ((IFace) dev).getFace().ordinal()) {
                                if (dev instanceof IInsulatedRedstoneDevice
                                        && ((IInsulatedRedstoneDevice) dev).getInsulationColor(bc.getSideB()) == getInsulationColor(bc.getSideA())) render = true;
                                if (getInsulationColor(bc.getSideA()) == MinecraftColor.NONE) render = true;
                            }
                            if (!(dev instanceof IInsulatedRedstoneDevice)) {
                                render = true;
                                connected = true;
        ...
        public int getWeakPower(ForgeDirection side) {
            ...
            if (!RedstoneApi.getInstance().shouldWiresOutputPower(hasLoss(side))) return 0;
            if (new Vec3i(this).add(side).getBlock() instanceof BlockRedstoneWire) return 0;
            if (side == getFace() || side == getFace().getOpposite()) return 0;
            if (!OcclusionHelper.microblockOcclusionTest(getParent(), MicroblockShape.EDGE, 1, getFace(), side)) return 0;
    ...
    public static class PartRedwireFaceBundled extends PartRedwireFace implements IAdvancedBundledConductor, IConnectionListener {
        ...
        protected IIcon getWireIcon(ForgeDirection side, ForgeDirection face) {
            ...
            if (face == ForgeDirection.UP || face == ForgeDirection.DOWN) {
                if (side == ForgeDirection.UNKNOWN) {
                    ...
                    if ((s1 || s2) && !(s3 || s4)) return IconSupplier.wireBundledStraight1;
                    if (!(s1 || s2) && (s3 || s4)) return IconSupplier.wireBundledStraight2;
                    if (!s1 && !s2 && !s3 && !s4) return IconSupplier.wireBundledStraight1;
                }
                return IconSupplier.wireBundledCross;
            }
            
            if (side == face) return IconSupplier.wireBundledConnection;
            
            if (face == ForgeDirection.UP || face == ForgeDirection.WEST || face == ForgeDirection.NORTH) return IconSupplier.wireBundledSide1;
        ...
        public boolean canConnect(ForgeDirection side, IBundledDevice device, ConnectionType type) {
            ...
            if (!color.canConnect(device.getBundledColor(type == ConnectionType.STRAIGHT ? side.getOpposite()
                    : (type == ConnectionType.CLOSED_CORNER ? getFace() : getFace().getOpposite())))) return false;
        ...
        public boolean renderStatic(Vec3i translation, RenderHelper renderer, RenderBlocks renderBlocks, int pass) {
            ...
            // Sides
            if (s4 || s3) {
                if (s3 || (!s1 && !s2)) renderer.renderBox(new Vec3dCube(s3 ? (cornerConnect[d3.ordinal()] ? -height - size : -size) : 5 / 16D, y,
                        8 / 16D - width, 8 / 16D - width, height + size, 8 / 16D + width), IconSupplier.wire);
                if (s4 || (!s1 && !s2)) renderer.renderBox(new Vec3dCube(8 / 16D + width, y, 8 / 16D - width, s4 ? (cornerConnect[d4.ordinal()] ? 1
                        + height + size : 1 + size) : 11 / 16D, height + size, 8 / 16D + width), IconSupplier.wire);
                if (s1) renderer.renderBox(new Vec3dCube(8 / 16D - width, y, s1 ? (cornerConnect[d1.ordinal()] ? -height - size : -size) : 4 / 16D,
                        8 / 16D + width, height + size, 8 / 16D - width), IconSupplier.wire);
                if (s2) renderer.renderBox(
                        new Vec3dCube(8 / 16D - width, y, 8 / 16D + width, 8 / 16D + width, height + size, s2 ? (cornerConnect[d2.ordinal()] ? 1
