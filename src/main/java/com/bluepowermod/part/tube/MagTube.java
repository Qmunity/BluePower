...
public class MagTube extends PneumaticTube {
    ...
    protected void renderSide() {
        ...
        if (connections[0]) {
            ...
            t.setNormal(0, -1, 0);
            t.addVertexWithUV(min, minY, min, maxX, maxY);// minY
            t.addVertexWithUV(max, minY, min, minX, maxY);
            t.addVertexWithUV(max, minY, max, minX, minY);
            t.addVertexWithUV(min, minY, max, maxX, minY);
            
            t.setNormal(0, 1, 0);
            t.addVertexWithUV(min, max, min, maxX, maxY);// maxY
            t.addVertexWithUV(min, max, max, minX, minY);
            t.addVertexWithUV(max, max, max, minX, minY);
            t.addVertexWithUV(max, max, min, maxX, maxY);
            
            t.setNormal(-1, 0, 0);
            t.addVertexWithUV(min, min, min, maxX, maxY);// minX
            t.addVertexWithUV(min, min, max, minX, minY);
            t.addVertexWithUV(min, max, max, minX, minY);
            t.addVertexWithUV(min, max, min, maxX, maxY);
            
            t.setNormal(1, 0, 0);
            t.addVertexWithUV(max, min, min, maxX, minY);// maxX
            t.addVertexWithUV(max, max, min, minX, minY);
            t.addVertexWithUV(max, max, max, minX, maxY);
            t.addVertexWithUV(max, min, max, maxX, maxY);
            
        } else if (connections[4]) {
            t.setNormal(0, 0, 1);
            t.addVertexWithUV(min, min, max, maxX, maxY);// maxZ
            t.addVertexWithUV(max, min, max, minX, minY);
            t.addVertexWithUV(max, max, max, minX, minY);
            t.addVertexWithUV(min, max, max, maxX, maxY);
            
            t.setNormal(0, 0, -1);
            t.addVertexWithUV(min, min, min, maxX, maxY);// minZ
            t.addVertexWithUV(min, max, min, minX, maxY);
            t.addVertexWithUV(max, max, min, minX, minY);
            t.addVertexWithUV(max, min, min, maxX, minY);
            
            t.setNormal(0, 1, 0);
            t.addVertexWithUV(min, max, min, maxX, maxY);// maxY
            t.addVertexWithUV(min, max, max, minX, maxY);
            t.addVertexWithUV(max, max, max, minX, minY);
            t.addVertexWithUV(max, max, min, maxX, minY);
            
            t.setNormal(0, -1, 0);
            t.addVertexWithUV(min, min, min, maxX, minY);// minY
            t.addVertexWithUV(max, min, min, minX, maxY);
            t.addVertexWithUV(max, min, max, minX, maxY);
            t.addVertexWithUV(min, min, max, maxX, minY);
        }
        ...
        for (int i = 2; i < 16; i += 8) {
            if (connections[0]) {
                ...
                t.setNormal(0, 0, 1);
                t.addVertexWithUV(min, min, 1 - i / 16D, maxX, maxY);// maxZ
                t.addVertexWithUV(max, min, 1 - i / 16D, minX, maxY);
                t.addVertexWithUV(max, max, 1 - i / 16D, minX, minY);
                t.addVertexWithUV(min, max, 1 - i / 16D, maxX, minY);
                
                t.setNormal(0, 0, -1);
                t.addVertexWithUV(min, min, i / 16D, maxX, maxY);//minZ
                t.addVertexWithUV(min, max, i / 16D, minX, maxY);
                t.addVertexWithUV(max, max, i / 16D, minX, minY);
                t.addVertexWithUV(max, min, i / 16D, maxX, minY);
            } else if (connections[4]) {
                t.setNormal(-1, 0, 0);
                t.addVertexWithUV(i / 16D, min, min, maxX, maxY);// minX
                t.addVertexWithUV(i / 16D, min, max, minX, maxY);
                t.addVertexWithUV(i / 16D, max, max, minX, minY);
                t.addVertexWithUV(i / 16D, max, min, maxX, minY);
                
                t.setNormal(1, 0, 0);
                t.addVertexWithUV(1 - i / 16D, min, min, maxX, minY);// maxX
                t.addVertexWithUV(1 - i / 16D, max, min, minX, minY);
                t.addVertexWithUV(1 - i / 16D, max, max, minX, maxY);
                t.addVertexWithUV(1 - i / 16D, min, max, maxX, maxY);
            }
        }
    }
}
